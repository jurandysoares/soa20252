...
Faça a instalação de uma distribuição GNU/Linux voltada para servidor ou contêiner em uma máquina virtual (VM) do [Oracle VirtualBox](https://www.virtualbox.org/) com três (3) adaptadores de rede:

1.  Conectado no modo NAT, obtendo configuração de rede IP de maneira dinâmica via DHCP;
2.  Conectado no modo *host-only*, obtendo configuração de rede IP de maneira dinâmica via DHCP; e 
3.  Não conectado, com endereço IP fixo conforme a tabela abaixo. 

No laboratório de redes de computadores do campus, o adaptador 3 será configurado no modo bridge. 

Sua VM não deverá ter interface gráfica. No entanto, a instalação poderá ser gráfica.
